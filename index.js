...
* @Date:   2016-06-21 14:44:36
* @Last Modified by:   Alexandre-COUEDELO
* @Last Modified time: 2016-06-22 00:29:14
* @links:
*   - https://github.com/GitbookIO/gitbook/blob/master/lib/utils/logger.js
*/
...
Logger.prototype.ok = function(level) {
    ...
    if (arguments.length > 1) {
        this.logLn(level, color.green('OK >> ') + msg.trim().replace(/\n/g, color.green('\n>> ')));
    } else {
...
Logger.prototype.fail = function(level) {
    ...
    var msg = this.format.apply(this, args);
    if (arguments.length > 1) {
        this.logLn(level, color.red('ERROR >> ') + msg.trim().replace(/\n/g, color.red('\n>> ')));
    } else {
        this.log(level, color.red('ERROR') + '\n');
    }
};
